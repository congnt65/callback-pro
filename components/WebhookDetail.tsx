...
function HeaderRow({ name, value }: { name: string; value: string }) {
  return (
    <div className='flex gap-2 py-1 border-b border-[#21262d] text-xs'>
      <span style={{ color: '#8b949e', minWidth: 180 }} className='shrink-0 break-all'>{name}</span>
      <span style={{ color: '#e6edf3' }} className='break-all'>{value}</span>
    </div>
  )
}
...
export default function WebhookDetail({ request }: Props) {
  const [tab, setTab] = useState<Tab>('body')
  const [copied, setCopied] = useState(false)

  const headers = request.headers as Record<string, string>
  const queryParams = request.query_params as Record<string, string>
  ...
  const tabs: { id: Tab; label: string; count?: number }[] = [
    ...
    { id: 'query', label: 'Query', count: Object.keys(queryParams).length },
  ]

  return (
    <div className='flex flex-col h-full overflow-hidden'>
      {/* Request info bar */}
      <div style={{ borderColor: '#30363d', backgroundColor: '#161b22' }} className='px-4 py-3 border-b shrink-0'>
        <div className='flex items-center gap-2 mb-1'>
          <MethodBadge method={request.method} />
          <span style={{ color: '#e6edf3' }} className='text-sm font-mono truncate'>{request.path || '/'}</span>
        </div>
        <div className='flex items-center gap-3 text-xs' style={{ color: '#6e7681' }}>
          <span>{new Date(request.received_at).toLocaleString()}</span>
          {request.ip && <span>&bull; {request.ip}</span>}
        </div>
      </div>

      {/* Tabs */}
      <div style={{ borderColor: '#30363d' }} className='flex gap-0 border-b shrink-0 px-4'>
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
          ...
          <div>
            {request.body ? (
              <div className='relative'>
                <button
                  onClick={() => copy(prettyBody ?? request.body ?? '')}
                  style={{ color: copied ? '#3fb950' : '#8b949e', backgroundColor: '#21262d' }}
              ...
              <p style={{ color: '#8b949e' }} className='text-xs italic'>No body</p>
            )}
          </div>
        )}
        ...
        {tab === 'headers' && (
          <div>
            {Object.keys(headers).length === 0 ? (
              <p style={{ color: '#8b949e' }} className='text-xs italic'>No headers</p>
            ) : (
        ...
        {tab === 'query' && (
          <div>
            {Object.keys(queryParams).length === 0 ? (
              <p style={{ color: '#8b949e' }} className='text-xs italic'>No query parameters</p>
            ) : (
...
function MethodBadge({ method }: { method: string }) {
  ...
  return (
    <span style={{ color, borderColor: color }} className='text-xs font-bold px-1.5 py-0.5 rounded border shrink-0'>
      {method}
    </span>
  )
}